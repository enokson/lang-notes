...
pub fn get_words_group(db: &mut Db, id: &i32) -> Result<Option<Row>, String> {
    ...
    let data: Option<Row> = match error_msg!(db.query(sql.as_str(), &[id])) {
...
pub fn get(data: Data<AppData>, info: Query<Info>) -> HttpResponse {
    match error_msg!(data.db.try_lock()) {
        Ok(mut db) => match error_msg!(get_words_group(&mut db, &info.id)) {
            Ok(data) => {
